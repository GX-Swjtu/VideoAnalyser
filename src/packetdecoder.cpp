    ...
    while (!found && maxPackets-- > 0) {
        ...
        while (true) {
            ...
            if (sentTarget) {
                ...
                // 使用 libswresample 转换为 float 交错格式
                if (frame->format != AV_SAMPLE_FMT_FLT || av_sample_fmt_is_planar(static_cast<AVSampleFormat>(frame->format))) {
                    ...
                    if (ret >= 0 && swr) {
                        ...
                        if (ret >= 0) {
                            ...
                            swr_convert(swr, &outBuf, nbSamples,
